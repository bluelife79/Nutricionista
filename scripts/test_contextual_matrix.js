...
const DESSERT_OR_SOLID_CHEESE_RE =
  ...
  if (["yogur", "kefir", "skyr"].includes(testCase.id)) {
    ...
    assert(
      items.every((food) =>
        ...
        !DESSERT_OR_SOLID_CHEESE_RE.test(normalize(food.name)),
      ),
      `${testCase.id}: queso sólido o postre impropio para un fermentado: ${itemNames.join(" | ")}`,
    );
  }
